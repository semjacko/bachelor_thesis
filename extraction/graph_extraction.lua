...
extractor = require "luadb.extraction.moduleExtractor"  -- pouzit moduleExtractor!!! nie obycajny extractor!!!
...
local function myExtract(graph)
	...
	for _, edge in pairs(graph.modified_edges) do
		...
		if (
			not string.find(fNodeType, "comment") and not string.find(fNodeType, "metrics") and not string.find(fNodeType, "body") 
			and fNodeType ~= "directory" and fNodeType ~= "cyclomatic" and fNodeType ~= "lines of code" and not string.find(tNodeType, "comment") 
			and not string.find(tNodeType, "metrics") and not string.find(tNodeType, "body") and tNodeType ~= "directory" 
			and tNodeType ~= "cyclomatic" and tNodeType ~= "lines of code"
		) then
	...
	for _, node in pairs(graph.modified_nodes) do
		...
		if (
			not string.find(nodetype, "comment") and not string.find(nodetype, "metrics") and not string.find(nodetype, "body") 
			and nodetype ~= "directory" and nodetype ~= "cyclomatic" and nodetype ~= "lines of code"
		) then
...
local function insertNode(nodes_list, node)
	...
	-- dodatocne spracovanie kontretnych typov uzlov
...
-- vrati id statementu podla zadaneho textu
-- ak sa taky statement v grafe nie je, vrati -1
...
-- velmi skareda funkcia ktora prepaja hranami vnorene funkcie
...
-- velky graf nad celym priecinkom len s dolezitymi uzlami a hranami
local myExtractedGraph = myExtract(extractedGraph)

for _, fileNode in pairs(myExtractedGraph.nodes) do

	-- hladanie uzlov len typu "file" ktore su .lua subory (ak existuje astID tak to je .lua subor)
	if (fileNode.meta.type == 'file' and fileNode.data.astID) then
		...
		for _, edge in pairs(myExtractedGraph.edges) do
			...
			if (
				edge.from[1].meta.modulePath == filePath or edge.from[1].data.astID == fileAstID 
				or edge.to[1].meta.modulePath == filePath or edge.to[1].data.astID == fileAstID
			) then
		...
		-- ak cesta k suboru obsahuje retazec "/test" oznac subor ako isTest
		fileGraph._isTest = string.find(string.lower(fileNode.data.path), "/test", nil, true) ~= nil
		
		-- ak cesta k suboru obsahuje retazec "/spec" oznac subor ako isSpec 
		fileGraph._isSpec = string.find(string.lower(fileNode.data.path), "/spec", nil, true) ~= nil
		
		-- ak sa na zaciatku suboru nachadza retazec "#!" oznac subor ako isShebang
