...
#include <im.h>
#include <im_lib.h>		// fix: needed for IM_VERSION
#include <im_image.h>
#include <im_util.h>
...
int app_about_cb(Ihandle *self)
{
  IupMessagef("About", "IUPGLView 1.0\n"
                       ...
                       "  Renderer: %s\n"
                       "  Version: %s\n"
                       , IUP_VERSION, IM_VERSION,  // fix: IUP_RELEASE_VERSION renamed to IUP_VERSION
...
{
  /* IUP initialization */
  IupOpen(&argc, &argv);	// fix: call with parameters
  IupGLCanvasOpen();
